...
#Adding CC1 as a NAP as it's not in the list of NAPs:
...
#loading Operon data 
OperonData=read.delim(file="AllOperons_MassSpec_Combined.txt",header=T,sep="\t",quote="",stringsAsFactors = F)
...
PfamDNAbinding=read.table(file="PFAM_DNA_binding_list.txt",stringsAsFactors = F)$V1
...
write.table(Restrict,"PFAM_DNA_binding_list_present_MassSpecDataSet.txt",quote=F,sep="\t",row.names = F,col.names = F)



#Reloading the manually annotated table ( I removed obvious false positive domains such as AAA, original publication listed in methods) :
...
#There is a PFAM to GO Annotation corresponding to TFs : -GO:0003700-, we load it.
...
Arch_Res=as.data.frame(read_tblout("Archaea_EBMCdb2_restricted_toMassSpecMuller2020.txt"))
...
#This is the part for which user need to download proteomes from ncbi
...
write.fasta(Prot,getName(Prot),file.out = "Archaea_all_species_MassSpec_proteins.fa")
}
...
#Those are proteins for which no PFAM domains has been found
...
#Reloading
MSProtStats=read.table("Archaea_all_species_MassSpec_proteinsPpties.txt",sep="\t",header=T,quote="",stringsAsFactors = F)
...
write.table(MassSpecArchaea.m[which(MassSpecArchaea.m$OperonSize>1 & MassSpecArchaea.m$NAP==1),c(1:3,which(names(MassSpecArchaea.m)%in%c("Assembly","Organism",NAPsHMM)))],file="Naps_that_are_in_Operons.txt",row.names=F,sep="\t",quote=F)
...
ggsave(OperonDistrib,file="Comparison_Operon_TF_NAPs.pdf",width=3,height=3)
...
AllProteins=read.fasta("Archaea_all_species_MassSpec_proteins.fa")

TFProteins=AllProteins[which(getName(AllProteins)%in%TranscriptionFactors$ID)]

write.fasta(sequences = TFProteins,names = getName(TFProteins),file="Archaea_all_species_MassSpec_proteins_PFAMTF_only.fa")

#Not in pfam : 
NoPFAMProteins=AllProteins[which(getName(AllProteins)%in%CandidateNapsNoPfam$ID)]

write.fasta(sequences = NoPFAMProteins,names = getName(NoPFAMProteins),file="Archaea_all_species_MassSpec_proteins_NoPFAMProteins.fa")


TFDNAbinder=read.table("DNABinder_Archaea_all_species_MassSpec_proteins_PFAMTF_only.txt",header = T,sep="\t",stringsAsFactors = F,quote="")
TFquartile=quantile(TFDNAbinder$SVM.score,probs=seq(0,1,by=0.05))

NoPfamDNAbinder=read.table("DNABinder_Archaea_all_species_MassSpec_proteins_NoPFAMProteins.txt",header = T,sep="\t",stringsAsFactors = F,quote="")
...
DNAperSpecie=read.table("PerSpeciePFAM_abundancy.txt",header=T,sep="\t",quote="",stringsAsFactors = F)
...
#Merging TF and putative TF/NAPs (control only did not make it into manuscript)
...
write.table(ExportCandidates,file ="NAPs_candidates_all.tab" ,row.names=F,quote=F,sep="\t")

#Re load (to make sure there's not export / import issues)
...
write.table(CandidateCount,file="PerSpecie_CandidateNapCount_and_abundancy.txt",row.names=F,quote=F,sep="\t")


#Re-load the table
CandidateCount=read.table("PerSpecie_CandidateNapCount_and_abundancy.txt",header=T,sep="\t",quote="",stringsAsFactors = F)
...
############################################################################
#formating the Specie variable as factor to make it compatible with ggplot2
...
#PLOT of all candidates properties (unecessarily complicated certainly)
...
#####################################################
#PART IV : RELATIVE ABUNDANCY OF NAPS AND CANDIDATE NAPs
#####################################################

#Exporting a stacked bar, displaying how total NAP abundancy distributes among different NAPs
...
######################################################################
#Export a specific table showcasing how histone hits are very lowly expressed in halophilic species
######################################################################
